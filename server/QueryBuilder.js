...
module.exports = Jii.defineClass('QueryBuilder', /** @lends QueryBuilder.prototype */{
	...
	/**
	 * Constructor.
	 * @param {Jii.sql.Connection} connection the database connection.
	...
	insert(table, columns, params) {
		...
		_each(columns, (value, name) => {
			...
			if (value instanceof Expression) {
				...
				params[phName] = !_isArray(value) && _has(columnSchemas, name) ?
					columnSchemas[name].typecast(value) :
					value;
			}
		});

		return Promise.resolve('INSERT INTO ' + this.db.quoteTableName(table) +
	...
	renameTable(oldName, newName) {
		return Promise.resolve('RENAME TABLE ' + this.db.quoteTableName(oldName) +
			' TO ' + this.db.quoteTableName(newName));
	},
	...
	addPrimaryKey(name, table, columns) {
		...
		return Promise.resolve('ALTER TABLE ' + this.db.quoteTableName(table) + ' ADD CONSTRAINT ' +
	...
	alterColumn(table, column, type) {
		return Promise.resolve('ALTER TABLE ' + this.db.quoteTableName(table) + ' CHANGE ' +
			this.db.quoteColumnName(column) + ' ' + this.db.quoteColumnName(column) + ' ' + this.getColumnType(type));
	},
	...
	createIndex(name, table, columns, isUnique) {
		...
		return Promise.resolve((isUnique ? 'CREATE UNIQUE INDEX ' : 'CREATE INDEX ') + this.db.quoteTableName(name) +
	...
	buildCondition(condition, params) {
		...
		if (condition[0]) { // operator format: operator, operand 1, operand 2, ...
			...
			var method = _has(this._conditionBuilders, operator) ?
				this._conditionBuilders[operator] :
	...
	buildBetweenCondition(operator, operands, params) {
		...
		if (value2 instanceof Expression) {
			...
			phName2 = this.__static.PARAM_PREFIX + _size(params);
	...
	buildInCondition(operator, operands, params) {
		...
		var result = inValues.length > 1 ?
			column + ' ' + operator + ' (' + inValues.join(', ') + ')' :
			column + (operator === 'IN' ? '=' : '<>') + inValues[0];
