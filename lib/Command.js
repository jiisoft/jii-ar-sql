 ...
 */
Jii.defineClass('Jii.sql.Command', {
	 ...
	 */
	getRawSql: function () {
		if (Jii._.isEmpty(this.params)) {
			return this._sql;
		}

		// Quote values
		var params = {};
		Jii._.each(this.params, function (value, name) {
			params[name] = this.db.quoteValue(value);
		}.bind(this));

		// Format `key = ?`
		if (Jii._.has(params, 1)) {
			var sql = '';
			Jii._.each(this._sql.split('?'), function (part, i) {
				sql += (params[i] || '') + part;
			});
			return sql;
		}
		...
		// Format `:name = 'John'`
		var sql2 = this._sql;
		Jii._.each(params, function (value, name) {
			name = name.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&");
	...
	bindValues: function (values) {
		Jii._.extend(this.params, values);
		return this;
	},
	 ...
	 * @returns {Jii.when.<number>} number of rows affected by the execution.
	...
	execute: function () {
		...
		return this.db.exec(rawSql, 'execute').then(function(result) {
			...
			return Jii.when.reject(exception, rawSql);
		});
	},
	 ...
	 * @returns {Jii.when} the reader object for fetching the query result
	 * @throws Exception execution failed
	 */
	query: function () {
		// @todo
	 ...
	 * @returns {Jii.when} all rows of the query result. Each array element is an array representing a row of data.
	 ...
	 * @returns {Jii.when} the first row (in terms of an array) of the query result. False is returned if the query
	 * results in nothing.
	 * @throws Exception execution failed
	 */
	queryOne: function () {
	 ...
	 * @returns {Jii.when} the value of the first column in the first row of the query result.
	 ...
	 * @returns {Jii.when} the first column of the query result. Empty array is returned if the query results in nothing.
	 * @throws Exception execution failed
	 */
	 ...
	 * @param {string} method
	 * @returns {Jii.when} the method execution result
	 * @throws Exception if the query causes any problem
	 */
	_queryInternal: function (method) {
		var rawSql = this.getRawSql();

		//Jii.info(rawSql, 'Jii.sql.Command.query');

		//var token = rawSql;
		//Jii.beginProfile(token, 'Jii.sql.Command.query');
		return this.db.exec(rawSql, method).then(function(result) {
			//Jii.endProfile(token, __METHOD__);

			return result;
		}, function(exception) {
			//Jii.endProfile(token, 'Jii.sql.Command.query');
			//$this->db->getSchema()->handleException($e, $rawSql);
	...
	insert: function (table, columns) {
		...
		return this.db.getQueryBuilder().insert(table, columns, params).then(function(sql) {
			this.setSql(sql);
	...
	batchInsert: function (table, columns, rows) {
		return this.db.getQueryBuilder().batchInsert(table, columns, rows).then(function(sql) {
			this.setSql(sql);
			return this.execute();
		}.bind(this));
	},
	...
	update: function (table, columns, condition, params) {
		...
		return this.db.getQueryBuilder().update(table, columns, condition, params).then(function(sql) {
			this.setSql(sql);
			this.bindValues(params);
	...
	delete: function (table, condition, params) {
		...
		return this.db.getQueryBuilder().delete(table, condition, params).then(function(sql) {
			this.setSql(sql);
			this.bindValues(params);
			return this.execute();
		}.bind(this));
	...
	createTable: function (table, columns, options) {
		...
		return this.db.getQueryBuilder().createTable(table, columns, options).then(function(sql) {
			this.setSql(sql);
			return this.execute();
		}.bind(this));
	},
	...
	renameTable: function (table, newName) {
		return this.db.getQueryBuilder().renameTable(table, newName).then(function(sql) {
			this.setSql(sql);
			return this.execute();
		}.bind(this));
	},
	...
	dropTable: function (table) {
		return this.db.getQueryBuilder().dropTable(table).then(function(sql) {
			this.setSql(sql);
			return this.execute();
		}.bind(this));
	},
	...
	truncateTable: function (table) {
		return this.db.getQueryBuilder().truncateTable(table).then(function(sql) {
			this.setSql(sql);
			return this.execute();
		}.bind(this));
	},
	 ...
	 * @param {string} type the column type. [[\Jii.sql.QueryBuilder.getColumnType()]] will be called
	 * to convert the give column type to the physical one. For example, `string` will be converted
	 * as `varchar(255)`, and `string not null` becomes `varchar(255) not null`.
	 * @returns {static} the command object itself
	 */
	addColumn: function (table, column, type) {
		return this.db.getQueryBuilder().addColumn(table, column, type).then(function(sql) {
			this.setSql(sql);
			return this.execute();
		}.bind(this));
	},
	...
	dropColumn: function (table, column) {
		return this.db.getQueryBuilder().dropColumn(table, column).then(function(sql) {
			this.setSql(sql);
			return this.execute();
		}.bind(this));
	},
	...
	renameColumn: function (table, oldName, newName) {
		return this.db.getQueryBuilder().renameColumn(table, oldName, newName).then(function(sql) {
			this.setSql(sql);
			return this.execute();
		}.bind(this));
	},
	...
	alterColumn: function (table, column, type) {
		return this.db.getQueryBuilder().alterColumn(table, column, type).then(function(sql) {
			this.setSql(sql);
			return this.execute();
		}.bind(this));
	},
	...
	addPrimaryKey: function (name, table, columns) {
		return this.db.getQueryBuilder().addPrimaryKey(name, table, columns).then(function(sql) {
			this.setSql(sql);
			return this.execute();
		}.bind(this));
	},
	...
	dropPrimaryKey: function (name, table) {
		return this.db.getQueryBuilder().dropPrimaryKey(name, table).then(function(sql) {
			this.setSql(sql);
			return this.execute();
		}.bind(this));
	},
	...
	addForeignKey: function (name, table, columns, refTable, refColumns, deleteOption, updateOption) {
		...
		return this.db.getQueryBuilder().addForeignKey(name, table, columns, refTable, refColumns, deleteOption, updateOption).then(function(sql) {
			this.setSql(sql);
			return this.execute();
		}.bind(this));
	},
	...
	dropForeignKey: function (name, table) {
		return this.db.getQueryBuilder().dropForeignKey(name, table).then(function(sql) {
			this.setSql(sql);
			return this.execute();
		}.bind(this));
	},
	...
	createIndex: function (name, table, columns, unique) {
		...
		return this.db.getQueryBuilder().createIndex(name, table, columns, unique).then(function(sql) {
			this.setSql(sql);
			return this.execute();
		}.bind(this));
	},
	...
	dropIndex: function (name, table) {
		return this.db.getQueryBuilder().dropIndex(name, table).then(function(sql) {
			this.setSql(sql);
			return this.execute();
		}.bind(this));
	},
	...
	resetSequence: function (table, value) {
		...
		return this.db.getQueryBuilder().resetSequence(table, value).then(function(sql) {
			this.setSql(sql);
			return this.execute();
		}.bind(this));
	},
	...
	checkIntegrity: function (check, schema, table) {
		...
		return this.db.getQueryBuilder().checkIntegrity(check, schema, table).then(function(sql) {
			this.setSql(sql);
			return this.execute();
		}.bind(this));
	}
