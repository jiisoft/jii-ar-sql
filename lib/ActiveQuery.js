 ...
 */
Jii.defineClass('Jii.sql.ActiveQuery', {
	...
	prepare: function (builder) {
		...
		if (!Jii._.isEmpty(this._joinWith)) {
			this._buildJoinWith();
			this._joinWith = null;    // clean it up to avoid issue https://github.com/jiisoft/jii2/issues/2687
		}
		...
		if (Jii._.isEmpty(this._select) && !Jii._.isEmpty(this._join)) {
			for (var alias in this._from) {
				if (!this._from.hasOwnProperty(alias)) {
					continue;
				}

				var table = this._from[alias];
		...
		return Jii.when.resolve().then(function() {
			...
			return Jii.when.resolve().then(function() {
				...
				return viaQuery.one().then(function(model) {
					...
					var viaModels = model === null ? [] : [model];
	...
	populate: function (rows) {
		...
		if (!Jii._.isEmpty(this._join) && this.indexBy === null) {
	 ...
	 *
	 * @param {[]} _with the relations to be joined. Each array element represents a single relation.
	...
	joinWith: function (_with, eagerLoading, joinType) {
		eagerLoading = eagerLoading || true;
		joinType = joinType || '';
	...
	_buildJoinWith: function () {
		...
		Jii._.each(this._joinWith, Jii._.bind(function (config) {
			...
			this._joinWithRelation(new this.modelClass(), _with, joinType);
		...
		if (!Jii._.isEmpty(join)) {
			...
			this._join = Jii._.isEmpty(this._join) ? join : Jii._.merge(this._join, join);
		}
	},
	...
	_joinWithRelations: function (model, _with, joinType) {
		...
		Jii._.each(_with, Jii._.bind(function (callback, name) {
			...
			if (!Jii._.has(relations, fullName)) {
				...
				if (!Jii._.isEmpty(relation.joinWith)) {
	...
	_getJoinType: function (joinType, name) {
		if (Jii._.isArray(joinType) && Jii._.has(joinType, name)) {
	...
	_getQueryTableName: function (query) {
		...
		if (Jii._.isEmpty(query.getFrom())) {
			...
			for (var alias in _from) {
				if (_from.hasOwnProperty(alias)) {
					if (Jii._.isString(alias)) {
						return [_from[alias], alias];
					} else {
						break;
					}
				}
	...
	viaTable: function (tableName, link, callable) {
		...
		var relation = new Jii.sql.ActiveQuery(this.primaryModel.className(), {
			from: [tableName],
			link: link,
			multiple: true,
			asArray: true
	 ...
	 * @returns {Jii.sql.ActiveQuery} the query object itself
	 */
	setInverseOf: function() {
		return this.inverseOf.apply(this, arguments);
	},
	...
	findFor: function (name, model) {
		...
		var related = this.multiple ? this.all() : this.one();
	...
	populateRelation: function (name, primaryModels) {
		...
		return Jii.when.resolve().then(function() {
			...
			return this.all().then(function(models) {
				...
				Jii._.each(primaryModels, function (primaryModel, i) {
					var keys = primaryModel.get(Jii._.values(link)[0]);
	...
	_populateInverseRelation: function (primaryModels, models, primaryName, name) {
		...
		if (relation.multiple) {
			...
			if (this.multiple) {
				Jii._.each(primaryModels, Jii._.bind(function (primaryModel, i) {
					Jii._.each(primaryModel[primaryName], function (m, j) {
	...
	_buildBuckets: function (models, link, viaModels, viaLink, checkMultiple) {
		...
		if (viaModels !== null) {
			var map = {};
			var viaLinkKeys = Jii._.values(viaLink);
	...
	_indexBuckets: function (buckets, indexBy) {
		...
		Jii._.each(buckets, function (models, key) {
			...
			Jii._.each(models, function (model) {
				var index = Jii._.isString(indexBy) ? model[indexBy] : indexBy.call(null, model);
	...
	_prefixKeyColumns: function (attributes) {
		if (!Jii._.isEmpty(this._join) || !Jii._.isEmpty(this._joinWith)) {
			...
			if (Jii._.isEmpty(this._from)) {
				...
				for (var a in this._from) {
					if (this._from.hasOwnProperty(a)) {
						if (!Jii._.isString(a)) {
							alias = this._from[a];
						}
						break;
			...
			Jii._.each(models, Jii._.bind(function (model) {
				...
				if (value !== null) {
					if (Jii._.isArray(value)) {
						values = Jii._.merge(values, value);
					} else {
						values.push(value);
					}
				}
	 ...
	 */
	setAsArray: function() {
		return this.asArray.apply(this, arguments);
	},
	 ...
	 */
	setWith: function() {
		return this.with.apply(this, arguments);
	},
	...
	findWith: function (_with, models) {
		...
		Jii._.each(relations, Jii._.bind(function (relation, name) {
			if (relation.asArray === null) {
				// inherit asArray from primary query
				relation.asArray(this._asArray);
			}
	...
	_normalizeRelations: function (model, _with) {
		...
		Jii._.each(_with, Jii._.bind(function (callback, name) {
			...
			if (childName) {
				relation._with[childName] = callback;
