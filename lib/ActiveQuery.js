...
Jii.defineClass('Jii.sql.ActiveQuery', /** @lends Jii.sql.ActiveQuery.prototype */{
	...
	prepare(builder) {
		...
		if (Jii._.isEmpty(this._select) && !Jii._.isEmpty(this._join)) {
			var isBreak = false;
			Jii._.each(this._from, (table, alias) => {
				if (isBreak) {
					return;
				}
	...
	_buildJoinWith() {
		...
		Jii._.each(this._joinWith, config => {
			...
			if (Jii._.isArray(eagerLoading)) {
				Jii._.each(_with, (callback, name) => {
	...
	_joinWithRelation(parent, child, joinType) {
		...
		if (!Jii._.isEmpty(child.link)) {
			...
			var on = [];
			Jii._.each(child.link, (parentColumn, childColumn) => {
		...
		if (!Jii._.isEmpty(child.getJoin())) {
			Jii._.each(child.getJoin(), join => {
				this._join.push(join);
			});
		}
		if (!Jii._.isEmpty(child.getUnion())) {
			Jii._.each(child.getUnion(), union => {
	...
	findFor(name, model) {
		...
		return (this.multiple ? this.all() : this.one()).then(related => {
			...
			if (this.multiple) {
				Jii._.each(related, (relatedModel, i) => {
		...
		return Promise.resolve().then(() => {
			...
			if (primaryModels.length === 1 && !this.multiple) {
				return this.one().then(model => {
					...
					Jii._.each(primaryModels, (primaryModel, i) => {
	...
	_filterByModels(models) {
		...
		if (Jii._.size(attributes) === 1) {
			...
			Jii._.each(models, model => {
				var v = {};
				Jii._.each(this.link, (link, attribute) => {
	...
	findWith(_with, models) {
		...
		/** @typedef {Jii.sql.ActiveQuery} relation */
		Jii._.each(relations, (relation, name) => {
