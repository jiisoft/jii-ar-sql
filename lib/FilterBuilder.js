...
Jii.defineClass('Jii.sql.FilterBuilder', /** @lends Jii.sql.FilterBuilder.prototype */{
    ...
    filterHashCondition(row, condition) {
		return Jii._.every(condition, (value, column) => {
    ...
    filterAndCondition(row, operator, operands) {
        ...
        return Jii._[method](operands, operand => {
    ...
    filterInCondition(row, operator, operands) {
		...
		return Jii._.some(values, value => {
    ...
    filterLikeCondition(operator, operands, params) {
		...
		var parts = [];
		Jii._.each(values, value => {
			var phName = null;
			if (value instanceof Jii.sql.Expression) {
				Jii._.each(value.params, (n, v) => {
					params[n] = v;
				});
