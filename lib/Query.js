...
Jii.defineClass('Jii.sql.Query', {
	...
	__static: {
		 ...
		 * @param {Jii.sql.Query} from the source query object
		 * @return {Jii.sql.Query} the new Query object
		 */
		createFromQuery: function(from) {
	...
	createCommand: function (db) {
		...
		return db.getQueryBuilder().build(this).then(function(buildParams) {
	 ...
	 * Jii._.each(query.batch(), Jii._.bind(function(rows) {
	 ...
	 * Jii._.each(query.each(), Jii._.bind(function(row) {
     * }, this));
	 * ```
	 *
	 * @param {number} batchSize the number of records to be fetched in each batch.
	...
	all: function (db) {
		...
		return this.createCommand(db).then(function(command) {
	 ...
	 * @returns {Jii.when.promise} number of records
	 */
	count: function (q, db) {
		q = q || '*';
		db = db || null;

		return this._queryScalar('COUNT(' + q + ')', db).then(function(result) {
			return Jii._s.toNumber(result);
		});
	},
	 ...
	 * @param {Jii.sql.Connection} db the database connection used to generate the SQL statement.
	 * If this parameter is not given, the `db` application component will be used.
	 * @returns {Jii.when.promise} the sum of the specified column values
	 */
	sum: function (q, db) {
		db = db || null;

		return this._queryScalar('SUM(' + q + ')', db).then(function(result) {
			return Jii._s.toNumber(result);
		});
	},
	 ...
	 * @param {Jii.sql.Connection} db the database connection used to generate the SQL statement.
	 * If this parameter is not given, the `db` application component will be used.
	 * @returns {Jii.when.promise} the average of the specified column values.
	 */
	average: function (q, db) {
		db = db || null;

		return this._queryScalar('AVG(' + q + ')', db).then(function(result) {
			return Jii._s.toNumber(result);
		});
	},
	 ...
	 * @param {Jii.sql.Connection} db the database connection used to generate the SQL statement.
	 * If this parameter is not given, the `db` application component will be used.
	 * @returns {Jii.when.promise} the minimum of the specified column values.
	 */
	min: function (q, db) {
		db = db || null;

		return this._queryScalar('MIN(' + q + ')', db).then(function(result) {
			return Jii._s.toNumber(result);
		});
	},
	 ...
	 * @param {Jii.sql.Connection} db the database connection used to generate the SQL statement.
	 * If this parameter is not given, the `db` application component will be used.
	 * @returns {Jii.when.promise} the maximum of the specified column values.
	 */
	max: function (q, db) {
		db = db || null;

		return this._queryScalar('MAX(' + q + ')', db).then(function(result) {
			return Jii._s.toNumber(result);
		});
	},
	 ...
	 * @param {Jii.sql.Connection} db the database connection used to generate the SQL statement.
	 * If this parameter is not given, the `db` application component will be used.
	 * @returns {Jii.when} whether the query result contains any row of data.
	 */
	exists: function (db) {
		db = db || null;
	...
	_queryScalar: function (selectExpression, db) {
		...
		return this.createCommand(db).then(function(command) {
			...
			if (Jii._.isEmpty(this._groupBy) && Jii._.isEmpty(this._union) && !this._distinct) {
				return command.queryScalar();
			} else {
	 ...
	 * in MySQL, the option 'SQL_CALC_FOUND_ROWS' can be used.
	 * @returns {Jii.sql.Query} the query object itself
	 */
	select: function (columns, option) {
		option = option || null;

		if (!Jii._.isArray(columns)) {
			columns = Jii._s.words(columns, ',');
		}
	 ...
	 * @returns {Jii.sql.Query} the query object itself
	 */
	setSelectOption: function(option) {
		this._selectOption = option;
		return this;
	},
	...
	addSelect: function (columns) {
		if (!Jii._.isArray(columns) && !Jii._.isObject(columns)) {
			columns = Jii._s.words(columns, ',');
		}
		if (this._select === null) {
			this._select = columns;
		} else {
			this._select = Jii._.isArray(columns) ?
				this._select.concat(columns) :
				Jii._.merge(columns, this._select);
		}
		return this;
	},
	...
	distinct: function (value) {
		value = !Jii._.isUndefined(value) ? value : true;
	...
	from: function (tables) {
		if (Jii._.isString(tables)) {
			tables = Jii._s.words(tables, ',');
		}
		this._from = tables;
		return this;
	},
	 ...
	 * @param {string|Jii.sql.Query} sql the SQL statement to be appended using UNION
	...
	addParams: function (params) {
		if (!Jii._.isEmpty(params)) {
			if (Jii._.isEmpty(this._params)) {
				this._params = params;
			} else {
				this._params = {};
				Jii._.each(params, Jii._.bind(function(value, name) {
	...
	_filterCondition: function (condition) {
		if (!Jii._.isPlainObject(condition) && !Jii._.isArray(condition)) {
			return condition;
		}

		if (!condition[0]) {
			// hash format: 'column1': 'value1', 'column2': 'value2', ...
			Jii._.each(condition, Jii._.bind(function(value, name) {
		...
		switch (operator.toUpperCase()) {
			...
			case 'AND':
			case 'OR':
				Jii._.each(condition, Jii._.bind(function(operand, i) {
			...
			case 'BETWEEN':
			case 'NOT BETWEEN':
				if (Jii._.has(condition, 1) && Jii._.has(condition, 2)) {
			...
			default:
				if (Jii._.has(condition, 1) && this._isEmpty(condition[1])) {
					return [];
				}
		}
	...
	_isEmpty: function (value) {
		return value === '' || (Jii._.isArray(value) && value.length === 0) || value === null || (Jii._.isString(value) && Jii._s.trim(value) === '');
	},
	...
	addOrderBy: function (columns) {
		...
		if (this._orderBy === null) {
			...
			this._orderBy = Jii._.extend(this._orderBy, columns);
		}
		return this;
	},

	_normalizeOrderBy: function (columns) {
		if (Jii._.isPlainObject(columns)) {
			return columns;
		}
		...
		Jii._.each(columns, Jii._.bind(function(column) {
			column = Jii._s.trim(column);
