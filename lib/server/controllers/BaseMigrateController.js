...
Jii.defineClass('Jii.sql.controllers.BaseMigrateController', /** @lends Jii.sql.controllers.BaseMigrateController.prototype */{
    ...
    _actionUpInternal(limit) {
        return this._getNewMigrations().then(migrations => {
            ...
            Jii._.each(migrations, migration => {
    ...
    actionHistory(context) {
        ...
        return this._getMigrationHistory(limit).then(migrations => {
            ...
            Jii._.each(migrations, (time, version) => {
     ...
     */
    _parseFields() {
        Jii._.each(this.fields, (field, index) => {
            var chunks = field.split(/\s?:\s?/);
            var property = chunks.shift();

            Jii._.each(chunks, (chunk, i) => {
                if (!chunk.match(/^(.+?)\(([^)]+)\)/)) {
                    chunks[i] += '()';
                }
            });
