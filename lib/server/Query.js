...
Jii.defineClass('Jii.sql.Query', /** @lends Jii.sql.Query.prototype */{
	 ...
	 * Jii._.each(query.batch(), function(rows) {
	 ...
	 * Jii._.each(query.each(), function(row) {
     * }.bind(this));
	 * ```
	 *
	 * @param {number} batchSize the number of records to be fetched in each batch.
	...
	_filterCondition: function (condition) {
		...
		if (!condition[0]) {
			// hash format: 'column1': 'value1', 'column2': 'value2', ...
			Jii._.each(condition, function(value, name) {
		...
		switch (operator.toUpperCase()) {
			...
			case 'AND':
			case 'OR':
				Jii._.each(condition, function(operand, i) {
	...
	_normalizeOrderBy: function (columns) {
		...
		Jii._.each(columns, function(column) {
