...
Jii.defineClass('Jii.sql.QueryBuilder', /** @lends Jii.sql.QueryBuilder.prototype */{
	...
	build(query, params) {
		...
		return query.prepare(this).then(query => {
			...
			return Promise.all([
				...
				clauses = Jii._.filter(clauses, sqlPart => {
					return !!sqlPart;
				});
	...
	buildOrderBy(columns) {
		...
		var orders = [];
		Jii._.each(columns, (direction, name) => {
	...
	buildColumns(columns) {
		...
		columns = Jii._.map(columns, column => {
			if (column instanceof Jii.sql.Expression) {
	...
	buildHashCondition(condition, params) {
		...
		Jii._.each(condition, (value, column) => {
