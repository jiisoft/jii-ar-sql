 ...
 */
Jii.defineClass('Jii.sql.BaseSchema', {
	 ...
	 * @return {Jii.when} the metadata for all tables in the database.
	 * Each array element is an instance of [[TableSchema]] or its child class.
	 */
	loadTableSchemas: function (schema, refresh) {
		schema = schema || '';
		refresh = refresh || false;

		return this.loadTableNames(schema, refresh).then(function(tableNames) {
			var tables = [];
			var promises = [];

			Jii._.each(tableNames, function (name) {
				if (schema !== '') {
					name = schema + '.' + name;
				}

				var promise = this.loadTableSchema(name, refresh).then(function(table) {
					if (table !== null) {
						tables.push(table);
					}
			...
			return Jii.when.all(promises).then(function() {
				return tables;
			});
		}.bind(this));
	 ...
	 * @return {Jii.when} all table names in the database.
	 */
	loadTableNames: function (schema, refresh) {
		schema = schema || '';
		refresh = refresh || false;

		if (Jii._.has(this._tableNames, schema) && !refresh) {
			return Jii.when.resolve(this._tableNames[schema]);
		}
	 ...
	 */
	loadTableSchema: function (name, refresh) {
		refresh = refresh || false;

		if (Jii._.has(this._tables, name) && !refresh) {
			return Jii.when.resolve(this._tables[name]);
		}
	...
	quoteValue: function (str) {
		if (Jii._.isUndefined(str) || str === null) {
			return 'NULL';
		}
	...
	quoteTableName: function (name) {
		...
		Jii._.each(parts, function (part, i) {
