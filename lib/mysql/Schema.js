...
Jii.defineClass('Jii.sql.mysql.Schema', {
	...
	_findColumns: function (table) {
		...
		return this.db.createCommand(sql).queryAll().then(function(columns) {
			if (columns === false) {
				return Jii.when.reject(new Jii.sql.SqlQueryException('Can not get metadata of table `' + table.name + '`'));
			}
	...
	_getCreateTableSql: function (table) {
		...
		return this.db.createCommand(sql).queryOne().then(function(row) {
			if (row === false) {
				return Jii.when.reject(new Jii.sql.SqlQueryException('Can not get CREATE TABLE sql for table `' + table.name + '`'));
			}

			return Jii._.has(row, 'Create Table') ?
				row['Create Table'] :
				Jii._.values(row)[1];
		});
	},
