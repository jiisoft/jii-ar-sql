...
Jii.defineClass('Jii.sql.ActiveRecord', {
	...
	__static: {
			...
			return Jii.when.resolve().then(function() {
				...
				return this.primaryKey().then(function(primaryKey) {
					if (primaryKey.length > 0) {
						var pk = primaryKey[0];
						if (!Jii._.isEmpty(query.join) || !Jii._.isEmpty(query.joinWith)) {
		 ...
		 */
		find: function () {
			return Jii.createObject(Jii.sql.ActiveQuery.className(), [this.className()]);
		},
	...
	attributes: function () {
		return this.__static.getTableSchema().then(function(schema) {
			return Jii._.keys(schema.columns);
		});
	},
