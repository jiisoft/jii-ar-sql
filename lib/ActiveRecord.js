...
Jii.defineClass('Jii.sql.ActiveRecord', /** @lends Jii.sql.ActiveRecord.prototype */{
	...
	_insertInternal: function (attributes) {
		...
		return this.beforeSave(true).then(function(bool) {
			...
			var db = this.__static.getDb();
			return db.createCommand().insert(this.__static.tableName(), values).then(function(insertInfo) {
				if (!insertInfo) {
					return false;
				}
				...
				var changedAttributes = Jii._.transform(values, function(result, num, key) {
					result[key] = null;
				});
	...
	_deleteInternal: function () {
		...
		return this.beforeDelete().then(function(bool) {
			...
			/*var lock = this.optimisticLock();
			if (lock !== null) {
				condition[lock] = this.lock;
			}*/
			return this.__static.deleteAll(condition).then(function(result) {
				/*if (lock !== null && !result) {
