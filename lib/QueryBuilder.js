 ...
 */
Jii.defineClass('Jii.sql.QueryBuilder', {
	...
	build: function(query, params) {
		...
		return query.prepare(this).then(function(query) {
			params = Jii._.merge(params, query.getParams());
			return Jii.when.all([
				...
				clauses = Jii._.filter(clauses, function(sqlPart) {
					return !!sqlPart;
				});

				var sql = clauses.join(this.separator);

				return this.buildUnion(query.getUnion(), params).then(function(union) {
					if (union !== '') {
	...
	batchInsert: function(table, columns, rows) {
		...
		Jii._.each(rows, function(row) {
			...
			Jii._.each(row, function(value, i) {
				if (!Jii._.isArray(value) && Jii._.has(columnSchemas, columns[i])) {
		...
		Jii._.each(columns, function(name, i) {
	...
	update: function(table, columns, condition, params) {
		...
		Jii._.each(columns, function(value, name) {
			if (value instanceof Jii.sql.Expression) {
				...
				params[phName] = !Jii._.isArray(value) && Jii._.has(columnSchemas, name) ? columnSchemas[name].typecast(value) : value;
			}
		}.bind(this));

		var sql = 'UPDATE ' + this.db.quoteTableName(table) + ' SET ' + lines.join(', ');

		return this.buildWhere(condition, params).then(function(where) {
	...
	createTable: function(table, columns, options) {
		...
		var cols = [];
		Jii._.each(columns, function(type, name) {
		...
		return Jii.when.resolve(options === null ? sql : sql + ' ' + options);
	},
	...
	renameTable: function(oldName, newName) {
		return Jii.when.resolve('RENAME TABLE ' + this.db.quoteTableName(oldName) +
	...
	dropTable: function(table) {
		return Jii.when.resolve('DROP TABLE ' + this.db.quoteTableName(table));
	},
	...
	addPrimaryKey: function(name, table, columns) {
		if (Jii._.isString(columns)) {
			columns = Jii._s.words(columns, ',');
		}

		columns = Jii._.map(columns, function(col) {
			return this.db.quoteColumnName(col);
		}.bind(this));

		return Jii.when.resolve('ALTER TABLE ' + this.db.quoteTableName(table) + ' ADD CONSTRAINT ' +
	...
	truncateTable: function(table) {
		return Jii.when.resolve('TRUNCATE TABLE ' + this.db.quoteTableName(table));
	},
	...
	alterColumn: function(table, column, type) {
		return Jii.when.resolve('ALTER TABLE ' + this.db.quoteTableName(table) + ' CHANGE ' +
	...
	createIndex: function(name, table, columns, isUnique) {
		...
		return Jii.when.resolve((isUnique ? 'CREATE UNIQUE INDEX ' : 'CREATE INDEX ') + this.db.quoteTableName(name) +
	...
	dropIndex: function(name, table) {
		return Jii.when.resolve('DROP INDEX ' + this.db.quoteTableName(name) + ' ON ' + this.db.quoteTableName(table));
	},
	...
	checkIntegrity: function(check, schema, table) {
		check = Jii._.isBoolean(check) ? check : true;
		schema = schema || '';
		table = table || '';
	...
	getColumnType: function(type) {
		...
		if (matches !== null ) {
			if (Jii._.has(this.typeMap, matches[1])) {
				return this.typeMap[matches[1]].replace(/\(.+\)/, '(' + matches[2] + ')') + matches[3];
			}
		} else {
			var matches2 = /^(\w+)\s+/.exec(type);
			if (matches2 !== null && Jii._.has(this.typeMap, matches2[1])) {
	...
	buildSelect: function(columns, params, distinct, selectOption) {
		...
		if (Jii._.isEmpty(columns)) {
			return Jii.when.resolve(select + ' *');
		}

		var normalizeColumns = [];
		var promises = [];
		Jii._.each(columns, function(column, i) {
			if (column instanceof Jii.sql.Expression) {
				normalizeColumns.push(column.expression);
				params = Jii._.extend(params, column.params);
			} else if (column instanceof Jii.sql.Query) {
				var promise = this.build(column, params).then(function(buildParams) {
	...
	buildFrom: function(tables, params) {
		if (Jii._.isEmpty(tables)) {
			return Jii.when.resolve('');
		}

		return this._quoteTableNames(tables, params).then(function(tables) {
	...
	buildJoin: function(joins, params) {
		if (Jii._.isEmpty(joins)) {
			return Jii.when.resolve('');
		}

		var promises = [];
		joins = Jii._.clone(joins);

		Jii._.each(joins, function(join, i) {
			if (!Jii._.isArray(join) || join.length < 1) {
			...
			var joinType = join[0];
			var table = join[1];
			if (!Jii._.isArray(table) && !Jii._.isObject(table)) {
				table = [table];
			}

			var promise = this._quoteTableNames(table, params).then(function(tables) {
				table = Jii._.values(tables)[0];

				joins[i] = joinType + ' ' + table;

				if (join[2]) {
					return this.buildCondition(join[2], params).then(function(condition) {
	...
	_quoteTableNames: function(tables, params) {
		...
		Jii._.each(tables, function(table, i) {
			if (table instanceof Jii.sql.Query) {

				var promise = this.build(table, params).then(function(buildResult) {
					var sql = buildResult[0];

					params = Jii._.extend(params, buildResult[1]);
					tables[i] = '(' + sql + ') ' + this.db.quoteTableName(i);
				}.bind(this));
				promises.push(promise);
			} else if (Jii._.isString(i)) {
				if (table.indexOf('(') === -1) {
		...
		return Jii.when.all(promises).then(function() {
			return Jii._.isObject(tables) ? Jii._.values(tables): tables
		});
	},
	...
	buildWhere: function(condition, params) {
		return this.buildCondition(condition, params).then(function(where) {
			return Jii.when.resolve(where === '' ? '' : 'WHERE ' + where);
		});
	},
	...
	buildGroupBy: function(columns) {
		return Jii.when.resolve(Jii._.isEmpty(columns) ? '' : 'GROUP BY ' + this.buildColumns(columns));
	},
	...
	buildHaving: function(condition, params) {
		return this.buildCondition(condition, params).then(function(having) {
			return Jii.when.resolve(having ? 'HAVING ' + having : '');
		});
	},
	...
	buildOrderBy: function(columns) {
		...
		var orders = [];
		Jii._.each(columns, function(direction, name) {
		...
		return Jii.when.resolve('ORDER BY ' + orders.join(', '));
	},
	...
	buildLimit: function(limit, offset) {
		...
		return Jii.when.resolve(Jii._s.ltrim(sql));
	},
	...
	_hasLimit: function(limit) {
		...
		return !Jii._.isNaN(limit) && limit >= 0;
	},
	...
	_hasOffset: function(offset) {
		...
		return !Jii._.isNaN(offset) && offset > 0;
	},
	...
	buildUnion: function(unions, params) {
		...
		Jii._.each(unions, function(union, i) {
			if (union.query instanceof Jii.sql.Query) {
				var promise = this.build(union.query, params).then(function(buildResult) {
	...
	buildColumns: function(columns) {
		if (Jii._.isString(columns)) {
			if (columns.indexOf('(') !== -1) {
				return Jii.when.resolve(columns);
			}

			columns = Jii._s.words(columns, ',');
		}

		columns = Jii._.map(columns, function(column) {
			if (column instanceof Jii.sql.Expression) {
		...
		return Jii.when.resolve(columns.join(', '));
	},
	...
	buildHashCondition: function(condition, params) {
		...
		Jii._.each(condition, function(value, column) {

			if (Jii._.isArray(value) || value instanceof Jii.sql.Query) {
				// IN condition
				var promise = this.buildInCondition('IN', [column, value], params).then(function(condition) {
	...
	buildAndCondition: function(operator, operands, params) {
		...
		Jii._.each(operands, function(operand) {
			if (Jii._.isArray(operand) || Jii._.isObject(operand)) {
				var promise = this.buildCondition(operand, params).then(function(condition) {
					parts.push(condition);
				});
				promises.push(promise);
			} else if (operand) {
		...
		return Jii.when.all(promises).then(function() {
			return Jii.when.resolve(parts.length > 0 ? '(' + parts.join(") " + operator + " (") + ')' : '');
		});
	},
	...
	buildBetweenCondition: function(operator, operands, params) {
		...
		if (value1 instanceof Jii.sql.Expression) {
			Jii._.each(value1.params, function(n, v) {
				params[n] = v;
			});
			phName1 = value1.expression;
		} else {
			phName1 = this.__static.PARAM_PREFIX + Jii._.size(params);
			params[phName1] = value1;
		}
		if (value2 instanceof Jii.sql.Expression) {
			Jii._.each(value2.params, function(n, v) {
				params[n] = v;
			});
			phName2 = value2.expression;
		} else {
			phName2 = this.__static.PARAM_PREFIX + Jii._.size(params);
			params[phName2] = value2;
		}

		return Jii.when.resolve(column + ' ' + operator + ' ' + phName1 + ' AND ' + phName2);
	},
	...
	buildInCondition: function(operator, operands, params) {
		...
		if (Jii._.isEmpty(values) || Jii._.isEmpty(column)) {
			return Jii.when.resolve(operator === 'IN' ? '0=1' : '');
		}

		if (values instanceof Jii.sql.Query) {
			// sub-query
			return this.build(values, params).then(function(buildResult) {
				var sql = buildResult[0];
				params = Jii._.extend(params, buildResult[1]);
				...
				Jii._.each(column, function(col, i) {
					if (col.indexOf('(') === -1) {
		...
		return Jii.when.resolve(result);
	},
	...
	_buildCompositeInCondition: function(operator, columns, values, params) {
		...
		return Jii.when.resolve('(' + columns.join(', ') + ') ' + operator + ' (' + vss.join(', ') + ')');
	},
	...
	buildLikeCondition: function(operator, operands, params) {
		...
		if (Jii._.isEmpty(values)) {
			return Jii.when.resolve(not ? '' : '0=1');
		}
		...
		return Jii.when.resolve(parts.join(andor));
	},
	...
	buildExistsCondition: function(operator, operands, params) {
		if (operands[0] instanceof Jii.sql.Query) {
			return this.build(operands[0], params).then(function(buildParams) {
				var sql = buildParams[0];
				params = Jii._.extend(params, buildParams[1]);
	 ...
	 */
	buildSimpleCondition: function (operator, operands, params) {
		if (operands.length !== 2) {
			throw new Jii.exceptions.InvalidParamException("Operator `" + operator + "` requires two operands.");
		}
		...
		if (value === null) {
			...
			Jii._.each(value.params, Jii._.bind(function(v, n) {
				params[n] = v;
			}, this));
